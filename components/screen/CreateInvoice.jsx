...
import {
  ...
  TextInput, Alert, TouchableOpacity, Modal, Switch, Platform, Dimensions, ActivityIndicator
} from 'react-native';
...
export default function CreateInvoice({ navigation, route }) {
  ...
  // State
  const [invoiceNumber, setInvoiceNumber] = useState(inputData?.invoiceNumber || '001');
  const [invoicePrefix, setInvoicePrefix] = useState(inputData?.invoicePrefix || `INVPG/${String(new Date().getMonth() + 1).padStart(2, '0')}`);
  const [invoiceDate, setInvoiceDate] = useState(inputData?.invoiceDate || today());
  const [invoiceValidity, setInvoiceValidity] = useState(inputData?.invoiceValidity || '');
  ...
  React.useEffect(() => {
    if (!inputData?.invoiceNumber) {
      getItemAsync('invoice').then(raw => {
        let maxNum = 0;
        if (raw) {
          const data = JSON.parse(raw);
          const arr = Array.isArray(data) ? data : [data];
          arr.forEach(q => {
            const num = parseInt(q.invoiceNumber, 10);
            if (!isNaN(num) && num > maxNum) maxNum = num;
          });
        }
        setInvoiceNumber(String(maxNum + 1).padStart(3, '0'));
        
        const months = ["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"];
        const currentMonth = months[new Date().getMonth()];
        setInvoicePrefix(`INVPG_${currentMonth}_`);
      }).catch(() => {});
    }
  }, [inputData]);
  ...
  const tax = productDetails.reduce((sum, p) => sum + (parseFloat(p.totalAmount) - parseFloat(p.taxableAmount || p.totalAmount) || 0), 0);
  const getGrandTotal = () => productDetails.reduce((sum, p) => sum + (parseFloat(p.totalAmount) || 0), 0);
  ...
  const openAddProduct = (product = null, index = null) => {
    ...
    setEditingProduct(product ? {
      ...
      unitPrice: String(product.unitPrice || product.rate || ''),
      gstRate: String(product.gstRate || '18'),
      discountRate: String(product.discount || product.discountRate || '0'),
      taxInclusive: product.taxInclusive || false,
      cess: String(product.cess || '0'),
      description: product.description || '',
      imageEnabled: !!product.productImage,
      productImage: product.productImage || null,
    } : {
      productName: '', description: '', hsnCode: '', unitPrice: '', quantity: '1', unit: 'PCS',
      discountRate: '0', discountType: '%', gstRate: '18', taxInclusive: false, cess: '0',
      imageEnabled: false, productImage: null,
    });
  ...
  const handleSave = async () => {
    ...
    const data = {
      ...(inputData?.quotationNumber ? { quotationNumber: inputData.quotationNumber, quotationPrefix: inputData.quotationPrefix } : {}),
      invoiceDate, invoicePrefix, invoiceNumber, 
      supplierDetails, buyerDetails, shipToDetails: finalShipTo, bankDetails,
      termsAndConditions: termsAndConditions.length > 0 ? termsAndConditions[0] : '',
      productDetails,
    ...
    try {
      setIsSaving(true);
      await addAndSave({ propertyName: 'invoice', newValue: data, propertyCheck: null });
      navigation.replace('InvoicePdfViewer', { data });
    } catch {
      alert.show('Error', 'Could not save invoice.', { type: 'error' });
      setIsSaving(false);
    }
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: bg }]}>
      ...
      <SafeAreaView style={{ backgroundColor: HEADER_BG, zIndex: 10 }} edges={['top']}>
        ...
        <View style={styles.headerStatsRow}>
          <View style={styles.statBox}>
            ...
            <TouchableOpacity style={styles.statValRow} onPress={() => openCalendar()}>
              <CalendarIcon size={13} color="#F6AD55" />
              <Text style={[styles.statInput, { paddingVertical: 2 }]}>{invoiceDate}</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.statBox}>
            ...
            <View style={styles.statValRow}>
              ...
              <TextInput style={[styles.statInput, { minWidth: 60, maxWidth: 85 }]} value={invoicePrefix} onChangeText={setInvoicePrefix} />
              <TextInput style={[styles.statInput, { flex: 1 }]} value={invoiceNumber} onChangeText={setInvoiceNumber} keyboardType="numeric" />
            </View>
          </View>
        </View>
      </SafeAreaView>
        ...
        <View style={{ marginTop: 24 }}>
          ...
          <View style={[styles.listContainer, { backgroundColor: cardBg, borderColor: cardBorder }]}>
            <View style={{ backgroundColor: cardBg }}>
              ...
              <View style={{ padding: 16, alignItems: 'center', borderTopWidth: productDetails.length > 0 ? 1 : 0, borderTopColor: '#F8FAFC' }}>
                ...
                {tax > 0 && (
                  <View style={{ backgroundColor: isDark ? '#0A2418' : '#F0FDF4', borderRadius: 10, padding: 10, marginBottom: 6 }}>
                    ...
                    <View style={[styles.totalRow, { marginBottom: 4 }]}>
                      ...
                      <Text style={[styles.totalLabel, { color: '#059669', fontSize: 11 }]}>₹{(tax / 2).toLocaleString()}</Text>
                    </View>
                    <View style={[styles.totalRow, { marginBottom: 4 }]}>
                    ...
                    <View style={[styles.totalRow, { marginBottom: 0, borderTopWidth: 1, borderTopColor: isDark ? '#134E3A' : '#D1FAE5', paddingTop: 4 }]}>
                      ...
                      <Text style={[styles.totalLabel, { color: '#059669', fontWeight: '700', fontSize: 12 }]}>₹{tax.toLocaleString()}</Text>
                    </View>
                  </View>
                )}
                <View style={styles.grandTotalRow}>
                  <Text style={[styles.grandTotalLabel, { color: textMain }]}>Total Amount</Text>
      ...
      <View style={[styles.stickyFooter, { paddingBottom: insets.bottom > 0 ? insets.bottom + 10 : 24, backgroundColor: cardBg, borderTopColor: isDark ? '#2C2C2E' : '#E2E8F0' }]}>
        <View>
          <Text style={{ fontSize: 11, fontWeight: '900', color: textSub, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Grand Total</Text>
          <Text style={{ fontSize: 22, fontWeight: '900', color: textMain, letterSpacing: -0.5 }}>₹{totalAmount.toLocaleString()}</Text>
        </View>
        <TouchableOpacity style={[styles.savePdfBtn, isSaving && { opacity: 0.7 }]} onPress={handleSave} activeOpacity={0.8} disabled={isSaving}>
          <Text style={{ color: '#FFF', fontSize: 15, fontWeight: '800' }}>{isSaving ? 'Saving...' : 'Save Invoice'}</Text>
