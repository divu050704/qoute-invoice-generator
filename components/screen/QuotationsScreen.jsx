...
export default function QuotationsScreen({ navigation }) {
  ...
  const isFocused = useIsFocused(); // Ensure we refresh on focus
  ...
  const handleStatusUpdate = (item) => {
    Alert.alert(
      "Update Status",
      `Change status for this quotation?`,
      [
  ...
  const updateStorageStatus = async (targetItem, newStatus) => {
    try {
      const updatedList = savedQuotations.map((item) => {
      ...
      // Persist to Storage
      await setItemAsync("quotation", JSON.stringify(updatedList));
    } catch (error) {
  ...
  const QuotationComponent = ({ quotation }) => {
    ...
    const amount = `₹${calculateGrandTotal(quotation)}`;
    ...
    return (
      <TouchableOpacity 
        ...
        activeOpacity={0.85} 
        onPress={() => navigation.navigate("ViewQuotation", {data: quotation})}
      >
        <View style={styles.cardMain}>
          ...
          <View style={styles.cardRight}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => handleStatusUpdate(quotation)}
            >
