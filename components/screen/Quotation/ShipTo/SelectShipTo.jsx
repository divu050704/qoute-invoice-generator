...
export default function SelectShipTo({ navigation, route }) {
  ...
  async function getExistingShipTos() {
    try {
      const shipToJson = await getItemAsync("shipTos");
      if (!shipToJson) {
        setSavedShipTos([]);
        return;
      }
      ...
      console.warn("Error parsing shipTos from secure store", e);
      setSavedShipTos([]);
    }
  }
  ...
  async function persistShipTos(list) {
    try {
      await setItemAsync("shipTos", JSON.stringify(list || []));
    } catch (e) {
      console.warn("Failed to persist shipTos", e);
    }
  }
  ...
  const handleSelect = (shipTo) => {
    if (onSave) onSave(shipTo);
    navigation.goBack();
  };

  // handle edit - open AddSupplier with initial data and update on save
  const handleEdit = (shipTo, originalIndex) => {
    navigation.navigate("AddShipTo", {
      shipToDetails: shipTo,
      onSave: (savedShipTo) => {
  ...
  const ShipToDetailsComponent = ({ data, index, originalIndex }) => (
    <View
      style={styles.shipToCard}
      key={`${data?.firmName ?? "shipTo"}-${index}`}
    >
      <TouchableOpacity
        activeOpacity={0.85}
        onPress={() => handleSelect(data)}
        style={styles.shipToTouchable}
      >
        <View style={styles.shipToTop}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerTop}>
            <View style={styles.headerLeft}>
              ...
              <Text style={styles.headerTitle}>Select Shipping Details</Text>
            </View>
        ...
        >
          {filteredShipTos.length === 0 ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyTitle}>No ShipTos found</Text>
              <Text style={styles.emptySub}>
                {savedShipTos.length === 0
                  ? "You don't have any shipTos yet. Add one using + Add."
