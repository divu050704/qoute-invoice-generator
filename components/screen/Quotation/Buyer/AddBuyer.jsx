...
import {
  ArrowLeft, Building2, MapPin, Briefcase, Check, ChevronDown, CheckCircle2, ToggleLeft,
} from 'lucide-react-native';

// ── Shared constants (single source of truth) ───────────────────────────────
import { PRIMARY, ACCENT } from '../../../../src/constants/colors';
import {
  INDIAN_STATES, SERVICE_TYPES, GST_TREATMENTS,
  PROJECT_TYPES, ENVIRONMENTS, LEAD_SOURCES,
} from '../../../../src/constants';
...
export default function AddBuyer({ navigation, route }) {
  ...
  const existing = route?.params?.buyer || null;
  ...
  const [form, setForm] = useState({
    ...
    serviceType:  existing?.serviceType  || 'Artificial Vertical Garden',
    gstin:        existing?.gstin        || '',
    gstTreatment: existing?.gstTreatment || 'Registered Business',
    street:       existing?.street       || '',
    city:         existing?.city         || '',
    state:        existing?.state        || 'Delhi',
    pincode:      existing?.pincode      || '',
    projectType:  existing?.projectType  || 'Installation',
    environment:  existing?.environment  || 'Indoor',
    leadSource:   existing?.leadSource   || 'Google Search',
    leadStatus:   existing?.leadStatus   || 'Active',
  });
  ...
  const handleSave = async () => {
    ...
    if (!form.mobile.trim()) {
      Alert.alert('Required', 'Phone Number 1 is required.'); return;
    }
    try {
      await addAndSave({ propertyName: 'buyer', newValue: form, propertyCheck: 'companyName' });
      navigation.goBack();
    } catch {
      Alert.alert('Error', 'Could not save client details.');
    }
  };
  ...
  return (
    <View style={styles.root}>
      ...
      >
        ...
        {/* Business & Contact */}
        <Section icon={Building2} title="Business & Contact">
          <Field
            label="Company Name" value={form.companyName}
            onChange={set('companyName')} placeholder="e.g. Green Spaces Pvt Ltd" required
          />
          ...
          <View style={styles.row}>
            <Field label="Phone 1 *" value={form.mobile} onChange={set('mobile')} placeholder="+91" keyboard="phone-pad" required />
            <View style={{ width: 12 }} />
            <Field label="Phone 2" value={form.mobile2} onChange={set('mobile2')} placeholder="Optional" keyboard="phone-pad" />
          </View>
          <View style={styles.divider} />

          <Field
            label="Email ID" value={form.email}
            onChange={set('email')} placeholder="email@example.com" keyboard="email-address"
          />
          <View style={styles.divider} />

          <SelectField label="Service Type" value={form.serviceType} options={SERVICE_TYPES} onChange={set('serviceType')} />
          <View style={styles.divider} />

          <View style={styles.row}>
            <Field label="GSTIN" value={form.gstin} onChange={set('gstin')} placeholder="22AAAAA0000A1Z5" />
            <View style={{ width: 12 }} />
            <SelectField label="GST Treatment" value={form.gstTreatment} options={GST_TREATMENTS} onChange={set('gstTreatment')} />
          </View>
        </Section>
        ...
        <Section icon={Briefcase} title="Project Details">
          <View style={styles.row}>
            <SelectField label="Project Type" value={form.projectType} options={PROJECT_TYPES} onChange={set('projectType')} />
            <View style={{ width: 12 }} />
            <SelectField label="Environment" value={form.environment} options={ENVIRONMENTS} onChange={set('environment')} />
          </View>
          <View style={styles.divider} />
          <SelectField label="Lead Source" value={form.leadSource} options={LEAD_SOURCES} onChange={set('leadSource')} />
        </Section>

        {/* Lead Status */}
        <Section icon={ToggleLeft} title="Lead Status">
          <View style={styles.row}>
            {[{ key: 'Active', emoji: '😊' }, { key: 'Inactive', emoji: '😔' }].map(({ key, emoji }) => (
              <TouchableOpacity
                key={key}
                activeOpacity={0.8}
                onPress={() => set('leadStatus')(key)}
                style={[styles.leadStatusBtn, form.leadStatus === key && (key === 'Active' ? styles.leadStatusBtnActive : styles.leadStatusBtnInactive)]}
              >
                <Text style={{ fontSize: 18 }}>{emoji}</Text>
                <Text style={[styles.leadStatusText, form.leadStatus === key && styles.leadStatusTextActive]}>{key}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </Section>
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom > 0 ? insets.bottom + 6 : 14 }]}>
        <TouchableOpacity style={styles.saveBtn} onPress={handleSave} activeOpacity={0.85}>
          ...
          <Text style={styles.saveBtnText}>
            {existing ? 'Update Client' : 'Save & Create Quote'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  leadStatusBtn: {
    ...
    paddingVertical: 14, borderRadius: 12, marginHorizontal: 4,
    backgroundColor: INPUT_BG, borderWidth: 1.5, borderColor: BORDER,
  },
  leadStatusBtnActive: {
    backgroundColor: PRIMARY, borderColor: PRIMARY,
  },
  leadStatusBtnInactive: {
    backgroundColor: '#EF4444', borderColor: '#EF4444',
  },
  leadStatusText: { fontSize: 13, fontWeight: '700', color: LABEL },
  leadStatusTextActive: { color: '#FFF' },
});
